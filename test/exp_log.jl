...
function test_exp_log_dim(::Val{D}, ::Val{M}) where {D,M}
    ...
    @testset "D=$D, M=$M" begin
        ...
        @testset "log(exp(x)) ≈ x" begin
            ...
            x = ChenSignatures.Tensor{Float64,D,M}()
            ChenSignatures._zero!(x) 
            ...
            # Compute g = exp(x)
            g = similar(x)
            ChenSignatures.exp!(g, x) # This requires the new implementation in src/dense_tensors.jl
        ...
        @testset "exp(log(g)) ≈ g" begin
            ...
            x_temp = ChenSignatures.Tensor{Float64,D,M}()
            ChenSignatures._zero!(x_temp)
            for k in 1:M
            ...
            g = similar(x_temp)
            ChenSignatures.exp!(g, x_temp) 
            
            # Compute x = log(g)
            x = similar(g)
            ChenSignatures.log!(x, g)
            
            # Compute h = exp(x)
            h = similar(g)
            ChenSignatures.exp!(h, x)
